...
def file_name(file_dir="D:/CB/deapProject/result/model_result/totalStatistic_result/"):
...
theta_shape = conv3_theta.get_shape().as_list()
alpha_shape = conv3_alpha.get_shape().as_list()
beta_shape = conv3_beta.get_shape().as_list()
gamma_shape = conv3_gamma.get_shape().as_list()

theta_flat = tf.reshape(conv3_theta, [-1, theta_shape[1] * theta_shape[2] * theta_shape[3]])
alpha_flat = tf.reshape(conv3_alpha, [-1, theta_shape[1] * theta_shape[2] * theta_shape[3]])
beta_flat = tf.reshape(conv3_beta, [-1, theta_shape[1] * theta_shape[2] * theta_shape[3]])
gamma_flat = tf.reshape(conv3_gamma, [-1, theta_shape[1] * theta_shape[2] * theta_shape[3]])
total_flat = tf.concat([theta_flat,alpha_flat,beta_flat,gamma_flat], axis=1)  #(?, 32768)
total_reshape = tf.reshape(total_flat, [-1, 1, 32, 1024])

total_flat = attention(total_reshape, fc_size)
...
print("acc:", mean_acc, "recall:", mean_recall, "precision:", mean_precision, "f1_score:", mean_f1, "auc:", mean_auc,
      "save path: D:/CB/deapProject/result/cnn_result/interBand_result/", save_file_name)
if save_file_name not in files:
    ...
    book.save("D:/CB/deapProject/result/cnn_result/interBand_result/" + save_file_name)

rexcel = open_workbook(
    "D:/CB/deapProject/result/cnn_result/interBand_result/" + save_file_name)  # 用wlrd提供的方法读取一个excel文件
...
excel.save("D:/CB/deapProject/result/cnn_result/interBand_result/" + save_file_name)
