...
def decompose(file):
    ...
    for trial in range(40):
        ...
        for channel in range(32):
            ...
            base_theta_DE = (compute_DE(base_theta[0:384]))
            base_alpha_DE =(compute_DE(base_alpha[0:384]))
            base_beta_DE =(compute_DE(base_beta[0:384]))
            base_gamma_DE =(compute_DE(base_gamma[0:384]))
            ...
            for index in range(60):
                DE_theta =np.append(DE_theta,compute_DE(theta[index*3*frequency:(index+1)*3*frequency]))
                DE_alpha =np.append(DE_alpha,compute_DE(alpha[index*3*frequency:(index+1)*3*frequency]))
                DE_beta =np.append(DE_beta,compute_DE(beta[index*3*frequency:(index+1)*3*frequency]))
                DE_gamma =np.append(DE_gamma,compute_DE(gamma[index*3*frequency:(index+1)*3*frequency]))
...
#
# if __name__ == '__main__':
#     dataset_dir = "D:/CB/data_preprocessed_matlab/"
#
#     result_dir = "D:/CB/deapProject/DE_feature/"
#     if os.path.isdir(result_dir)==False:
#         os.makedirs(result_dir)
#
#     for file in os.listdir(dataset_dir):
...
#                                            "dominance_labels":dominance_labels,"liking_labels":liking_labels})

        trial_DE[0] = preprocessing.scale(trial_DE[0], axis=0, with_mean=True, with_std=True, copy=True)
        trial_DE[1] = preprocessing.scale(trial_DE[1], axis=0, with_mean=True, with_std=True, copy=True)
        trial_DE[2] = preprocessing.scale(trial_DE[2], axis=0, with_mean=True, with_std=True, copy=True)
        trial_DE[3] = preprocessing.scale(trial_DE[3], axis=0, with_mean=True, with_std=True, copy=True)
