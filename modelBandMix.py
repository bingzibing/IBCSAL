...
def file_name(file_dir="D:/CB/deapProject/result/modelBandMix_result/alpha_beta_gamma_result/"):
...
datasets_theta = datasets[:, 0:32].reshape(-1,input_height, input_width, input_channel_num)
datasets_alpha=datasets[:,32:64].reshape(-1,input_height, input_width, input_channel_num)
datasets_beta=datasets[:,64:96].reshape(-1,input_height, input_width, input_channel_num)
datasets_gamma=datasets[:,96:128].reshape(-1,input_height, input_width, input_channel_num)
...
def attention(x, ch, scope='attention', reuse=tf.AUTO_REUSE):
    with tf.variable_scope(scope, reuse=reuse):
        f = apply_conv2d(x, 1, 1, ch // 8, 1, name='f_conv')  # [bs, h, w, c']
        g = apply_conv2d(x, 1, 1, ch // 8, 1, name='g_conv')  # [bs, h, w, c']
        h = apply_conv2d(x, 1, 1, ch , 1, name='h_conv')  # [bs, h, w, c]
        # N = h * w
...
alpha_att,alpha_data=attention(conv3_alpha,conv_channel_num * 4)
beta_att, beta_data=attention(conv3_beta,conv_channel_num * 4)
gamma_att,gamma_data=attention(conv3_gamma,conv_channel_num * 4)

alpha_shape = alpha_data.get_shape().as_list()
beta_shape = beta_data.get_shape().as_list()
gamma_shape = gamma_data.get_shape().as_list()

alpha_flat=tf.reshape(alpha_data,[-1, alpha_shape[1] * alpha_shape[2] * alpha_shape[3]])
beta_flat=tf.reshape(beta_data,[-1, beta_shape[1] * beta_shape[2] * beta_shape[3]])
gamma_flat=tf.reshape(gamma_data,[-1, gamma_shape[1] * gamma_shape[2] * gamma_shape[3]])

alpha_beta_gamma_flat=tf.concat([alpha_flat,beta_flat,gamma_flat],axis=1)
total_fc=apply_fully_connect(alpha_beta_gamma_flat,alpha_beta_gamma_flat.shape[1],fc_size,name='total_fc')
        ...
        for epoch in range(training_epochs):
            ...
            for b in range(batch_num_per_epoch):
                ...
                _, c = session.run([optimizer, cost],
                                   feed_dict={alpha_in: alpha_batch,beta_in: beta_batch, gamma_in: gamma_batch, Y: batch_y, keep_prob: 1 - dropout_prob,
                                              phase_train: True})
                summary, _ = session.run([merged_summary_op, optimizer], feed_dict={alpha_in: alpha_batch,beta_in: beta_batch, gamma_in: gamma_batch, Y: batch_y, keep_prob: 1 - dropout_prob,
                                              phase_train: True})
            ...
            if (epoch % 1 == 0):
                ...
                for i in range(train_accuracy_batch_num):
                    ...
                    train_a, train_c = session.run([accuracy, cost],
                                                   feed_dict={alpha_in: train_alpha_batch,beta_in: train_beta_batch, gamma_in: train_gamma_batch,
                ...
                for j in range(test_accuracy_batch_num):
                    ...
                    test_a, test_c = session.run([accuracy, cost],
                                                 feed_dict={alpha_in: test_alpha_batch,beta_in: test_beta_batch, gamma_in: test_gamma_batch,
...
print("acc:", mean_acc, "save path: D:/CB/deapProject/result/modelBandMix_result/alpha_beta_gamma_result/", save_file_name)
if save_file_name not in files:
    ...
    book.save("D:/CB/deapProject/result/modelBandMix_result/alpha_beta_gamma_result/" + save_file_name)

rexcel = open_workbook(
    "D:/CB/deapProject/result/modelBandMix_result/alpha_beta_gamma_result/" + save_file_name)  # 用wlrd提供的方法读取一个excel文件
...
excel.save("D:/CB/deapProject/result/modelBandMix_result/alpha_beta_gamma_result/" + save_file_name)
