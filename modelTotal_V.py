...
def file_name(file_dir="D:/CB/deapProject/result/stft_result/"):
...
dataset_dir = "D:/CB/p4_featureExperiment/feature/1D_kurtosis_with/kurt_s"
###load training set
input_file = "%02d"%sub
...
# labels = np.transpose(labels)   #(1,800)
# labels = data_file[label_key]
# 2018-5-16 modified
...
def apply_conv2d(x, filter_height, filter_width, out_channels, kernel_stride,name='conv'):
    with tf.variable_scope(name):
        ...
        return tf.nn.selu(batch_norm_layer(conv))
...
# theta_fc = apply_fully_connect(theta_flat, theta_shape[1] * theta_shape[2] * theta_shape[3], fc_size, name="theta_fc")
# alpha_fc = apply_fully_connect(alpha_flat, alpha_shape[1] * alpha_shape[2] * alpha_shape[3], fc_size, name="alpha_fc")
# beta_fc = apply_fully_connect(beta_flat, beta_shape[1] * beta_shape[2] * beta_shape[3], fc_size, name="beta_fc")
# gamma_fc = apply_fully_connect(gamma_flat, gamma_shape[1] * gamma_shape[2] * gamma_shape[3], fc_size, name="gamma_fc")

# theta_fc_drop = tf.nn.dropout(theta_fc, keep_prob)
# alpha_fc_drop = tf.nn.dropout(alpha_fc, keep_prob)
# beta_fc_drop = tf.nn.dropout(beta_fc, keep_prob)
# gamma_fc_drop = tf.nn.dropout(gamma_fc, keep_prob)
# total_fc_drop=tf.concat([theta_fc_drop,alpha_fc_drop,beta_fc_drop,gamma_fc_drop],axis=1)   #(?,4096)
...
mean_recall=sum_recall/fold * 100
mean_precision=sum_precision/fold * 100
mean_f1=sum_f1/fold * 100
mean_auc=sum_auc/fold * 100

files = file_name()
save_file_name = arousal_or_valence + "_" + str(bands) + ".xls"
print("acc:", mean_acc, "recall:", mean_recall, "precision:", mean_precision, "f1_score:", mean_f1, "auc:", mean_auc,
      "save path: D:/CB/deapProject/result/stft_result/", save_file_name)
if save_file_name not in files:
    ...
    sheet1.write(0, 4, "f1_score")
    sheet1.write(0, 5, "auc")
    # sheet1.write(0,2,"without")

    for i in range(1, 33):
        sheet1.write(i, 0, "s" + "%02d" % i)
    # 保存Excel book.save('path/文件名称.xls')
    book.save("D:/CB/deapProject/result/stft_result//" + save_file_name)

rexcel = open_workbook(
    "D:/CB/deapProject/result/stft_result/" + save_file_name)  # 用wlrd提供的方法读取一个excel文件
rows = rexcel.sheets()[0].nrows  # 用wlrd提供的方法获得现在已有的行数
excel = copy(rexcel)  # 用xlutils提供的copy方法将xlrd的对象转化为xlwt的对象
table = excel.get_sheet(0)  # 用xlwt对象的方法获得要操作的sheet
row = rows
# if with_or_not=="with_base" else 2
table.write(sub, 1,  mean_acc)
table.write(sub, 2,  mean_recall)
table.write(sub, 3,  mean_precision)
table.write(sub, 4,  mean_f1)
table.write(sub, 5, mean_auc)
excel.save("D:/CB/deapProject/result/stft_result/" + save_file_name)
