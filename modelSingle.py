...
from modules import ln
import matplotlib.ticker as ticker

os.environ["CUDA_VISIBLE_DEVICES"] = "1"

def file_name(file_dir="D:/CB/deapProject/result/model_result/theta_result/"):
...
def plot_confusion_matrix(cm, labels_name):
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            plt.text(x=j,y=i,s=str('%.2f'%(cm[i,j])),va='center',ha='center',color='black',fontsize=10)
...
conv_3 = apply_conv2d(conv_2, kernel_height_3rd, kernel_width_3rd, conv_channel_num * 4,kernel_stride, name='conv3')# (?,1,32,256)
# conv_4 = apply_conv2d(conv_3, kernel_height_4th, kernel_width_4th, conv_channel_num, kernel_stride, name='conv4')
theta_att,theta_data=attention(conv_3,conv_channel_num * 4)


shape = theta_data.get_shape().as_list()
conv_4_flat=tf.reshape(theta_data,[-1, shape[1] * shape[2] * shape[3]])
...
for curr_fold in range(0, fold):
    ...
    cnn_test_x = datasets_theta[split]
    test_y = labels[split]

    split = np.array(list(set(indexes_list) ^ set(split_list)))
    cnn_train_x = datasets_theta[split]
    ...
    with tf.Session(config=config) as session:
                ...
                for j in range(test_accuracy_batch_num):
                    ...
                    test_a, test_c = session.run([accuracy, cost],
                                                 feed_dict={cnn_in: test_cnn_batch, Y: test_batch_y, keep_prob: 1.0,
                                                            phase_train: False})
                    attention_output, attention_c = session.run([theta_att, cost],
                                                    feed_dict={cnn_in: test_cnn_batch, Y: test_batch_y, keep_prob: 1.0})

                    attention_output.sum(axis=0)
                    print(attention_output.shape)


                    variables = ['Fp1', 'AF3', 'F7', 'F3', 'FC1', 'FC5', 'T7', 'C3', 'CP1', 'CP5', 'P7', 'P3',
                                 'Pz',
                                 'PO3', 'O1', 'Oz', 'O2', 'PO4', 'P4', 'P8', 'CP6', 'CP2', 'C4', 'T8', 'FC6',
                                 'FC2',
                                 'F4', 'F8', 'AF4', 'Fp2', 'Fz', 'Cz']
                    labels = ['Fp1', 'AF3', 'F7', 'F3', 'FC1', 'FC5', 'T7', 'C3', 'CP1', 'CP5', 'P7', 'P3',
                              'Pz',
                              'PO3', 'O1', 'Oz', 'O2', 'PO4', 'P4', 'P8', 'CP6', 'CP2', 'C4', 'T8', 'FC6',
                              'FC2',
                              'F4', 'F8', 'AF4', 'Fp2', 'Fz', 'Cz']
                    df = pd.DataFrame(np.array(attention_output[0]), columns=variables, index=labels)
                    fig = plt.figure()
                    ...
                    ax.set_xticklabels(['']+list(df.columns),rotation='vertical')
                    ax.set_yticklabels(['']+ list(df.index))
                    plt.savefig('theta_attention_4.jpg',dpi=500)
                    plt.show()
        ...
        pred_y = session.run(tf.argmax(y_, 1),
                             feed_dict={cnn_in: cnn_test_x, keep_prob: 1 - dropout_prob, phase_train: True})
        cm = confusion_matrix(np.argmax(test_y, 1), pred_y)
        print(cm)
        plot_confusion_matrix(cm, ['HV', 'LV'])
        plt.savefig('EEG_%s_%s_cm.jpg' % (sub, arousal_or_valence), dpi=500)
        plt.show()
...
print("acc:", mean_acc, "save path: D:/CB/deapProject/result/model_result/theta_result/", save_file_name)
if save_file_name not in files:
    ...
    book.save("D:/CB/deapProject/result/model_result/theta_result/" + save_file_name)

rexcel = open_workbook(
    "D:/CB/deapProject/result/model_result/theta_result/" + save_file_name)  # 用wlrd提供的方法读取一个excel文件
...
excel.save("D:/CB/deapProject/result/model_result/theta_result/" + save_file_name)
